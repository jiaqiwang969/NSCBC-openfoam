...
#include "velocityOutletNSCBCzFvPatchField.H"
...
template<class Type>
Foam::velocityOutletNSCBCzFvPatchField<Type>::velocityOutletNSCBCzFvPatchField
(
    const fvPatch& p,
...
Foam::velocityOutletNSCBCzFvPatchField<Type>::velocityOutletNSCBCzFvPatchField
(
    const velocityOutletNSCBCzFvPatchField& ptf,
    const fvPatch& p,
    const DimensionedField<Type, volMesh>& iF,
...
Foam::velocityOutletNSCBCzFvPatchField<Type>::velocityOutletNSCBCzFvPatchField
(
    const velocityOutletNSCBCzFvPatchField& ptpsf
)
:
...
Foam::velocityOutletNSCBCzFvPatchField<Type>::advectionSpeed() const
{
...
template<class Type>
Foam::tmp<Foam::scalarField>
Foam::velocityOutletNSCBCzFvPatchField<Type>::soundSpeed() const
{
...
template<class Type>
void Foam::velocityOutletNSCBCzFvPatchField<Type>::updateCoeffs()
{
	if (this->updated())
	{
		return;
	...
	if (lInf_ > 0)
	{
		...
		if
			...
			)
			{
				...
				forAll(Lund, facei)
				{
					...
					lws.xx() = 1.0 /(1.0 + aP[facei]*deltaT*deltaCoeffs[facei]);
					lws.xy() = 0.0;
					lws.xz() = 0.0;
					lws.yy() = 1.0 /(1.0 + aP[facei]*deltaT*deltaCoeffs[facei]);
					lws.yz() = 0.0;
					lws.zz() = 1.0 /(1.0 + (aP[facei]+cP[facei])/2.0*deltaT*deltaCoeffs[facei]);
				}
		...
		else if (ddtScheme == fv::backwardDdtScheme<scalar>::typeName)
		{
			...
			{
				symmTensor& lws = Lund[facei];
				lws.xx() = 1.5 /(1.5 + aP[facei]*deltaT*deltaCoeffs[facei]);
				lws.xy() = 0.0;
				lws.xz() = 0.0;
				lws.yy() = 1.5 /(1.5 + aP[facei]*deltaT*deltaCoeffs[facei]);
				lws.yz() = 0.0;
				lws.zz() = 1.5 /(1.5 +  (aP[facei]+cP[facei])/2.0*deltaT*deltaCoeffs[facei]);
			}
...
template<class Type>
void Foam::velocityOutletNSCBCzFvPatchField<Type>::write(Ostream& os) const
{
